      ...
      try {
        ...
        if (saved) {
          ...
          if (parsed.sessionId && parsed.examSet && parsed.leftSeconds > 0) {
            if (!cancelled) restoreSession(parsed);
            return;
          }
        }
      } catch {}
    ...
    return (
      <div>
        ...
        {part1.questions.map((q, idx) => {
          ...
          const savedLeft = questionTimerLeft[q.id];
          ...
          return (
            <div key={q.id} style={{ ...S.card, borderColor: isLocked && !part1Submitted ? "#fde68a" : "#f0f0f0", background: isLocked && !part1Submitted ? "#fffbeb" : "#fff" }}>
              ...
              <QuestionTimer questionId={q.id} limitSeconds={limit} savedLeft={savedLeft} onExpire={handleQuestionExpire} onTick={handleQuestionTick} locked={isLocked} />
