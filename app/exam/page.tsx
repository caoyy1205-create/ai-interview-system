...
// ─── 统一倒计时组件 SectionTimer ─────────────────────────────────────────────
// label: 显示文字（如"选择题"、"问答题"、"AI协作"）
// mode: "bar"=独立卡片(Part1用), "inline"=嵌入式小组件(Part2用)
function SectionTimer({ label, limitSeconds, savedLeft, onExpire, onTick, locked, mode = "bar" }: {
  label: string; limitSeconds: number; savedLeft?: number;
  onExpire: () => void; onTick?: (left: number) => void;
  locked: boolean; mode?: "bar" | "inline";
}) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${critical ? "#fca5a5" : urgent ? "#fde68a" : "#e5e7eb"}`,
      borderRadius: "8px", padding: "12px 20px", marginBottom: "20px",
      display: "flex", alignItems: "center", gap: "16px",
      boxShadow: (urgent || critical) ? "0 2px 8px rgba(220,38,38,0.12)" : "none",
      animation: critical ? "timerPulse 1s ease-in-out infinite" : "none",
    }}>
      <span style={{ fontSize: "13px", color: critical ? "#dc2626" : urgent ? "#92400e" : "#555", fontWeight: 600, whiteSpace: "nowrap" }}>
        ⏱ {label}
      </span>
      <span style={{ fontFamily: "monospace", fontSize: "22px", fontWeight: 800, color: critical ? "#dc2626" : urgent ? "#b45309" : "#111", letterSpacing: "0.05em" }}>
        {formatTime(left)}
      </span>
      <div style={{ flex: 1, height: "6px", background: "#e5e7eb", borderRadius: "3px", overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${pct * 100}%`, background: barColor, borderRadius: "3px", transition: "width 1s linear, background 0.5s" }} />
      </div>
      {critical && <span style={{ fontSize: "12px", color: "#dc2626", fontWeight: 700, whiteSpace: "nowrap" }}>⚠️ 即将结束！</span>}
      {urgent && !critical && <span style={{ fontSize: "12px", color: "#b45309", fontWeight: 600, whiteSpace: "nowrap" }}>请抓紧时间</span>}
    </div>
  );
}
...
export default function ExamPage() {
  ...
  // 选择题阶段 tick/expire
  const handleMcTick = useCallback((left: number) => {
    setMcTimerLeft(left);
    persistState({ mcTimerLeft: left });
  }, []);
  ...
  // 问答题阶段 tick/expire
  const handleEssayTick = useCallback((left: number) => {
    setEssayTimerLeft(left);
    persistState({ essayTimerLeft: left });
  }, []);

  const handleEssayExpire = useCallback(() => {
    // 问答题时间到：锁住问答题，自动提交 Part1
    setEssayLocked(true);
